...
async def get_sniper_analysis(ticker, language="ru"):
    ...
    if not indicators:
        indicators = {
            ...
            "pivot": "N/A", "s1": "N/A", "r1": "N/A", 
            "regime": "N/A", "safety": "N/A", "atr": "N/A"
        }
    ...
    # ГИБРИДНЫЙ СУПЕР-ПРОМТ (Math + Liquidity Hunter Template)
    prompt = f"""
    Ты — профессиональный аналитик Liquidity Hunter (Smart Money + Intraday).
    
    ВХОДНЫЕ ДАННЫЕ (ФАКТЫ ИЗ PYTHON):
    • Актив: {ticker.upper()} | Цена: ${curr_price}
    • Тренд: {indicators['trend']} | RSI (14): {indicators['rsi']}
    • Режим рынка: {indicators['regime']} (Safety: {indicators['safety']})
    
    РАСЧЕТНЫЕ УРОВНИ (ИСПОЛЬЗУЙ ИХ ОБЯЗАТЕЛЬНО):
    • Pivot (Баланс дня): ${indicators['pivot']}
    • Поддержка (S1): ${indicators['s1']}
    • Сопротивление (R1): ${indicators['r1']}
    
    ЗАДАЧА:
    Проведи анализ по стратегии "Liquidity Hunter" и дай сигнал.
    Структура ответа должна строго соответствовать пунктам ниже.
    
    СТРУКТУРА АНАЛИЗА (ШАГ ЗА ШАГОМ):
    
    1. 📊 Ключевые уровни: 
       Укажи S1, R1 и Pivot, которые я передал. Объясни, где цена относительно них.
    
    2. 🐋 Логика Маркетмейкера (MM):
       Используй концепции:
       - Liquidity Hunter 2.0: Где стопы? (За S1 или R1?)
       - Spoofing/Layering: Есть ли ложные стенки?
       - Accumulation: Есть ли признаки скрытого набора?
       
    3. 🔬 Order Book & Sentiment (Эмуляция):
       - Куда давит толпа? (Если толпа шортит у поддержки -> будет вынос вверх).
       - CVD: Есть ли дивергенции?
    
    4. 🧮 P-Score (Вероятность):
       Оцени вероятность сценария (0-100%). Если Regime=COMPRESSION, снижай балл (риск флэта).
       Liquidity Map: Где зона ликвидаций?
       
    5. 🎯 Снайперский Сигнал (Intraday):
       Дай ОДНО направление (Long или Short).
       - Вход: Строго рядом с расчетным уровнем ({indicators['s1']} или {indicators['r1']}). Отступ не более 0.5-1%.
       - Тейки: TP1 (Pivot), TP2 (R1/S1), TP3 (Moonbag).
       - Стоп: За уровнем ликвидности.
    
    ФОРМАТ ОТВЕТА (HTML, CLEAN UI):
    
    📊 <b>{ticker.upper()} | Liquidity Hunter Setup</b>
    💰 Цена: ${curr_price} ({change}%)
    
    📡 <b>Индикаторы & Уровни:</b>
       ▪️ RSI: <b>{indicators['rsi']}</b> ({indicators['trend']})
       ▪️ Pivot: <b>${indicators['pivot']}</b>
       ▪️ S1: <b>${indicators['s1']}</b> | R1: <b>${indicators['r1']}</b>
    
    1️⃣ <b>Логика Маркетмейкера</b>
    ▪️ <b>Фаза:</b> [Накопление / Манипуляция / Распределение]
    ▪️ <b>План ММ:</b> [Опиши: сбор ликвидности за уровнем {indicators['s1']} или {indicators['r1']}?]
    ▪️ <b>Ловушки:</b> [Где расставлены капканы?]
    
    2️⃣ <b>Sentiment & P-Score</b>
    ▪️ Толпа: [Лонгует или Шортит?]
    ▪️ <b>P-Score:</b> <b>[XX]%</b>
    ▪️ <b>Liquidity Map:</b> [Где деньги?]
    
    🎯 <b>Снайперский план</b>
    🔵 Тип: <b>[LONG / SHORT]</b> (Limit)
    🚪 Вход: <b>[Цена]</b> (Zone: {indicators['s1']} / {indicators['r1']})
