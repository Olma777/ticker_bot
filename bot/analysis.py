...
async def get_daily_briefing(market_data=None):
    try:
        ...
        system_prompt = f""" 
Ты — элитный крипто-аналитик фонда. Твоя задача — просканировать рынок (на основе переданных цен топ-активов) и найти кандидатов на ПАМП в секторах AI, RWA, Layer-2, DePIN. 
 
ВХОДНЫЕ ДАННЫЕ (Рынок сейчас): 
{market_data_str} 
 
ИНСТРУКЦИЯ: 
Проведи глубокий анализ и выяви в каждом из секторов (AI, RWA, L2, DePIN) по одной монете, которая готовится к пампу. 
Используй критерии: 
1. Технический анализ: Фаза аккумуляции, формирование дна, бычьи дивергенции. 
2. Liquidity Hunter 2.0: Сбор стопов, ложные пробои ("фитили"), рост OI при консолидации. 
...
• <b>Следы ММ:</b> (Напр: Скрытый набор позиций, спуфинг в стакане) 
• 🎯 <b>Сигнал:</b> LONG 
  └ Вход: $... 
  └ TP: $... / $... 
  └ SL: $... 
...
... (аналогично для DePIN) 
 
⚠️ <i>Дисклеймер: Это аналитика вероятностей, соблюдайте риск-менеджмент.</i> 
"""
