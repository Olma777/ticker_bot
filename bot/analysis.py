...
async def get_sniper_analysis(ticker, language="ru"):
    ...
    # НОВЫЙ ПРОМТ (Trend Level PRO + Liquidity Hunter Template)
    prompt = f"""
    Ты — профессиональный алгоритмический трейдер (Trend Level PRO + Liquidity Hunter).
    
    ВХОДНЫЕ ДАННЫЕ (PYTHON CALCULATION):
    • Актив: {ticker.upper()} | Цена: ${curr_price}
    • Режим рынка: {indicators['regime']} (Важно! Если COMPRESSION — ожидай вынос стопов)
    • Статус безопасности: {indicators['safety']}
    • Ключевые уровни (Pivot Logic):
      - Сопротивление (R1): ${indicators['r1']}
      - Поддержка (S1): ${indicators['s1']}
    • RSI (14): {indicators['rsi']} | Тренд: {indicators['trend']}
    
    ТВОЯ ЗАДАЧА:
    Проведи анализ по жесткому алгоритму и выдай сигнал.
    
    АЛГОРИТМ АНАЛИЗА (ШАГ ЗА ШАГОМ):
    1. 📊 Ключевые уровни: Используй переданные S1 и R1. Это жесткие границы диапазона.
    
    2. 📈 Готовность к движению (Tech + Sentiment):
       - Если Regime = COMPRESSION: Цена сжата. ММ копит позицию. Высокий риск "Вертолетов" (сбор стопов с обеих сторон).
       - Если Regime = EXPANSION: Тренд сильный. Торгуй пробой или откат.
       - Sentiment (Эмуляция): Предположи действия толпы. Если цена у R1, а толпа шортит -> будет пробой вверх.
       
    3. 🐋 Действия Маркетмейкера (MM Strategies):
       - Liquidity Hunter: Где стопы? (Обычно за S1 для лонгов и за R1 для шортов).
       - Spoofing: Есть ли стенки?
    
    4. 🔬 Order Book & CVD Logic:
       - Куда заходит умный капитал?
       
    5. 🎯 ФИНАЛЬНЫЙ СИГНАЛ:
       - Дай ЧЕТКИЙ план (Long или Short).
       - Точка входа: Строго рядом с уровнем S1 (для лонга) или R1 (для шорта). Не давай вход "по рынку" в середине диапазона!
       - Стоп-лосс: За уровнем ликвидности.
    
    ФОРМАТ ОТВЕТА (HTML, CLEAN UI):

    📊 <b>{ticker.upper()} | Trend Level PRO Setup</b>
    💰 Цена: ${curr_price} ({change}%)
    
    📡 <b>Индикаторы v3.7:</b>
       ▪️ Режим: <b>{indicators['regime']}</b> ({indicators['safety']})
       ▪️ RSI: <b>{indicators['rsi']}</b> | Тренд: <b>{indicators['trend']}</b>
       ▪️ S1 (Поддержка): <b>${indicators['s1']}</b>
       ▪️ R1 (Сопротивление): <b>${indicators['r1']}</b>
    
    1️⃣ <b>Структура и Контекст</b>
    ▪️ Фаза: [Накопление/Распределение на основе Режима]
    ▪️ Анализ: [Кратко опиши ситуацию. Если Compression - предупреди о волатильности]

    2️⃣ <b>Ликвидность & MM Logic</b>
    ▪️ Цель ММ: [Сбор стопов за S1 или R1?]
    ▪️ Сценарий: [Опиши, как ММ будет обманывать толпу перед движением]
    
    🎯 <b>Снайперский план</b>
    🔵 Тип: <b>[LONG / SHORT]</b> (Limit)
    🚪 Вход: <b>[Цена]</b> (Строго от уровня!)
