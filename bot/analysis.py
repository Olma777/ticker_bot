...
async def get_sniper_analysis(ticker, language="ru"):
    ...
    # ФИНАЛЬНЫЙ ПРОМТ (Smart Money + Score Logic)
    prompt = f"""
    ...
    УРОВНИ (ОТСОРТИРОВАНЫ ПО БЛИЗОСТИ К ЦЕНЕ):
    • Поддержки (SUP): {indicators['supports_list']}
    • Сопротивления (RES): {indicators['resistances_list']}

    КРИТИЧЕСКИЕ ПРАВИЛА ВХОДА:
    1. 📏 ДИСТАНЦИЯ: Точка входа должна быть в пределах 3% от текущей цены. Не предлагай уровни на дне рынка!
    2. 🛑 RSI ФИЛЬТР: 
       - RSI выше 65 у RES = ЗАПРЕТ ЛОНГА (ищи Short).
       - RSI ниже 35 у SUP = ЗАПРЕТ ШОРТА (ищи Long).
    3. 💪 СИЛА УРОВНЯ (Score):
       - Score ≥ 3.0: Strong (Лимитный ордер).
       - Score ниже 1.0: Weak (Требует подтверждения / Stop-Limit).

    ВАЖНОЕ ТЕХНИЧЕСКОЕ ПРАВИЛО:
    • НИКОГДА не используй символы '<' или '>' в тексте анализа (например, НЕ пиши "RSI < 30").
    • Вместо этого пиши словами: "ниже 30", "выше 70".
    • Или используй безопасные символы: '«', '»', '≤', '≥'.
    • Иначе сообщение не отправится из-за ошибки HTML парсера!
    ...
    • Режим: <b>{indicators['regime']}</b> ({'Торгуй диапазон' if indicators['regime'] == 'NEUTRAL' or indicators['regime'] == 'COMPRESSION' else 'Торгуй по тренду'})

    1️⃣ <b>СТРУКТУРА & ЛОГИКА</b>
    ▪️ <b>Фаза:</b> [Накопление/Распределение?]
    ▪️ <b>Анализ:</b> [Цена у сильного или слабого уровня? Что говорит RSI?]

    2️⃣ <b>P-SCORE (ВЕРОЯТНОСТЬ)</b>
    ▪️ <b>P-Score:</b> <b>[РАССЧИТАЙ]%</b>
       Формула:
       • База: 50%
       • Режим: {'+20% (Expansion)' if indicators['regime'] == 'EXPANSION' else '-10% (Compression/Neutral)'}
       • Уровень: +15% (если Score > 3.0) или -20% (если Score < 1.0)
       • RSI: -15% (если против тренда/перегрет)

    🎯 <b>СНАЙПЕРСКИЙ ПЛАН</b>
    🚦 <b>Тип:</b> [LONG/SHORT] (Limit / Stop-Limit)
    🚪 <b>Вход:</b> <code>[Цена]</code> (Строго от ближайшего уровня! Макс. 3% от цены)

    🛡 <b>Стоп-лосс:</b>
       🔴 <code>[Цена]</code> (За зоной ликвидности)

    ✅ <b>Тейк-профиты:</b>
       🟢 TP1: <code>[Цена]</code> (Ближайшая встречная зона)
       🟢 TP2: <code>[Цена]</code> 
       
    ⚖️ <b>Совет:</b> Риск 1%. {'⚠️ ВНИМАНИЕ: Слабый уровень поддержки!' if indicators['s1_score'] < 1.0 else ''}
    """
