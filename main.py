...
# Убедитесь, что файлы prices.py и analysis.py находятся в той же папке
...
async def start_handler(message: Message):
    ...
    await message.answer(
        "👋 <b>Рад вас видеть! Я — ваш персональный консультант.</b>\n\n"
        "Я здесь, чтобы фильтровать шум и находить для вас реальные возможности.\n\n"
        "☕️ <b>Я взял на себя смелость запланировать для вас утренний брифинг на 09:00.</b>\n"
        "Так вы начнете день с понимания контекста, пока остальные только читают новости.\n\n"
        "<i>(Если график неудобен — скажите мне через команду /time, и я подстроюсь).</i>\n\n"
        "<b>Какие задачи я могу решить для вас сейчас?</b>\n"
        "🦅 <code>/sniper [тикер]</code> — Рассчитаю сделку и риски (SMC)\n"
        "🛡 <code>/audit [тикер]</code> — Изучу проект «под капотом»\n"
        "🌅 <code>/daily</code> — Введу в курс событий за 30 секунд",
...
async def time_handler(message: Message):
    ...
    current_setting = USER_SETTINGS.get(message.from_user.id, "Отключено")
    
    # Форматируем текущее время для отображения
    if isinstance(current_setting, int):
        status_text = f"<b>{current_setting:02d}:00</b>"
    else:
        status_text = "<b>Отключено</b> 🔕"

    await message.answer(
        f"⏰ <b>Настройка рассылки</b>\n"
        f"Ваше текущее время: {status_text}\n\n"
        "Выберите удобный час (по времени сервера/МСК), чтобы получать свежую аналитику:",
